...
parser.add_argument("--patchRadius", default=5, type=int, help="Radius of resource patches")
...
def calcCellSquare(side, upper_left=0):
    """
    This function returns a string that can specify the cells required for a square cell resource.

    Arguments: side - int, indicating length of a side of the square
    upperleft - cell that will be the upperleft corner of the grid (default: 0)
    """
    outstrs = []
    x_offset = upper_left%args.worldSize
    y_offset = upper_left/args.worldSize
    for i in range(y_offset, side+y_offset):
        outstr = str(i*args.worldSize + x_offset)
        outstr += ".."
        outstr += str(i*args.worldSize + x_offset + side-1)
        outstrs.append(outstr)

    return outstrs

def recurringSquares(size, sqrt_n):
    """
    WARNING! This function only works for square sizes that the world size is
    equally divisible by. Maybe not even all of them. Check that your desired
    configuration actually works.
    """
    outstrs = []
    for i in range(sqrt_n):
        for j in range(sqrt_n):
            start_cell = i*args.worldSize*2*size + j*2*size
            outstrs += calcCellSquare(size, start_cell)
